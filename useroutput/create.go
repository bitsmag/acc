...
// CreateBalance returns a printable to inform the user about the account balance
func CreateBalance(balance float64) types.Printable {
	...
	return types.Line{Line: "Balance is: " + formattedBalance, LineColor: color}
}

// CreateLog returns a slice of printables representing the logEntries plus additional separation printables (according to types.Order)
func CreateLog(entries []types.LogEntry, logOrder types.Order) []types.Printable {
	if len(entries) == 0 {
		return make([]types.Printable, 0)
	}

	var log []types.Printable
...
func createLogWithSeparatorLines(groupedLog map[string][]types.LogEntry) []types.Printable {
	logWithSeparatorLines := make([]types.Printable, 0)

	sortedKeys := sortKeys(groupedLog)
	for _, key := range sortedKeys {
		separatorLine := getSeparatorLine(key)
		logEntriesForKey := printiblyEntries(groupedLog[key])
		logEntriesForKeyWithSeparatorLine := append([]types.Printable{separatorLine}, logEntriesForKey...)
		logWithSeparatorLines = append(logWithSeparatorLines, logEntriesForKeyWithSeparatorLine...)
	}
...
func getSeparatorLine(key string) types.Printable {
	separatorLine := types.Line{Line: "======================================" + key, LineColor: "white"}
	return separatorLine
}

func printiblyEntries(entries []types.LogEntry) []types.Printable {
	printableEntries := make([]types.Printable, len(entries))
	for i, entry := range entries {
		line := logEntryLine(entry)
		color := logEntryColor(entry)
		printableEntries[i] = types.Line{Line: line, LineColor: color}
	}
...
func logEntryColor(entry types.LogEntry) string {
	var color string
	if entry.Amount > 0 {
		color = "green"
	} else {
